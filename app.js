...
function initSplashScreen() {
    ...
    const calendarPanel = document.getElementById('calendar-panel'); // Elemento del calendario

    if (!splash || !app || !logo || !calendarPanel) {
        console.error("Faltan elementos esenciales para el inicio (splash, app, logo o calendar-panel).");
        return;
    }
    ...
    // 2. Forzar al modal de cadencias a estar OCULTO
    if (cadenceModal) {
        cadenceModal.classList.add('oculto');
        cadenceModal.style.display = 'none';
    }
    ...
    const enterApp = () => {
        ...
        // SCROLL: Navegar al calendario suavemente
        setTimeout(() => {
            calendarPanel.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }, 100); // Pequeño retardo para asegurar que la app es visible

        // Cuando la transición del splash termine, lo ocultamos completamente
